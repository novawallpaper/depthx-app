...
const GOOGLE_CLIENT_ID = "452456583028-1186bibq60ggkl301h5j88sed7v04eof.apps.googleusercontent.com";
...
async function handleGoogleCredentialResponse(response) {
    // Send the raw token to the backend for real signature verification.
    // Never trust a frontend-only decode for anything sensitive (payments,
    // account creation, etc.) — it can be forged by anyone.
    try {
        ...
        if (!res.ok || !data.verified) {
            showToast("Sign-In failed — could not verify with server.");
            return;
        }
        ...
        showToast("Sign-In failed — backend unreachable.");
    }
}

function signOutGoogle() {
    localStorageSafeSet("isLoggedIn", "false");
...
function waitForGoogleThenInit() {
    ...
    googleInitAttempts++;
    if (googleInitAttempts > 20) { // ~5s max wait
        console.warn("Google Identity Services script did not load within 5s (slow/blocked network or script blocked).");
        showGoogleFallbackButton();
        return;
    }
    setTimeout(waitForGoogleThenInit, 250);
}
...
function initGoogleSignIn() {
    ...
    // Google's real button is rendered inside the full-screen login gate,
    // styled as a white pill to match the app's design.
    const btnContainer = document.getElementById("google-signin-btn-gate");
    if (btnContainer) {
    ...
    // If the user was already signed in during this session, try a
    // silent one-tap prompt so they don't have to click again.
    google.accounts.id.prompt();

    // Safety net: if Google's button never actually rendered into the
    // slot (wrong/unregistered origin, blocked script, ad-blocker, etc.)
    // show a manual fallback so the user is never stuck with an empty gap.
    setTimeout(() => {
...
function showGoogleFallbackButton() {
    ...
    const statusEl = document.getElementById("signin-status");
    if (statusEl) statusEl.innerText = "Real Google button didn't load — using fallback.";
}

function handleFallbackGoogleClick() {
    if (typeof google !== "undefined" && google.accounts && google.accounts.id) {
        // Re-try the real Google flow first
        google.accounts.id.prompt((notification) => {
            if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
                showToast("Google Sign-In blocked — check that this domain is added under Authorized JavaScript origins in Google Cloud Console.");
            }
        });
    } else {
        showToast("Google Sign-In script not available — check your connection.");
    }
}
